...
4. ONE CARD PER PRIORITY — MERGE HARD, ACROSS WORDING, CATEGORY AND ALTITUDE. The same priority shows up many times: in different words, in different categories, and at different altitudes — sometimes as the MOTIVATION or value ("stay physically capable for the long term") and separately as the ACTIVITY that serves it ("keep running, walking, moving"). Those are ONE priority: merge into a single card, and never carry both a "why" card and its matching activity card, or two cards a person would obviously live out as the same thing. Every card must be a distinct priority, with no overlap.
   CRITICAL — merge PARAPHRASES. Two candidates that describe the same underlying wish in different words are ONE card: merge them and pick the single clearest phrasing. For example "Master cooking", "Master cooking — go to a cooking school" and "Moving towards mastery in cooking" are ONE card (say "Master cooking"); "Travel as much as possible, including the longer trips" and "As much travel as possible, including longer immersive trips" are ONE card; "Renovate our home to make it a dream home" and "Home renovation done bit by bit" are ONE card. Before you add a card, check it against every card already on your list — if a reader would see two as the same aim, keep only one. A board with any visible repeats is a failure.
