...
const KEYS = {
  ...
  // from the narrow aspiration-capped set to the full candidate pool (people and
  // activities included), so v1/v2 caches are left unread and simply re-curated.
...
// ---- Self-heal: regenerate takeaways that were dropped at wrap-up ----

// A session's takeaway is generated by a fire-and-forget background call when the
// person wraps up (SessionContainer.generateAndStoreTakeaway). It has no retry
// and no persistence guarantee, so a reload or an immediate jump to the next
// session can abort it before it saves — leaving a completed session with no
// takeaway, silently. The dashboard then shows the generic "Last time you worked
// through X" fallback, and Vita (and the plan) lose that session entirely.
//
// This heals that on load: for every completed session that has a real
// conversation but no takeaway, we regenerate the summary from the stored
// transcript (the same /api/takeaway path used live) and persist it. Runs in the
// background so it never blocks the app, and is idempotent — once a takeaway
// exists the session is skipped, so a steady-state load makes zero model calls.
// `persistIfAbsent` writes only when the key is still missing, so it can never
// clobber a real takeaway that lands first. `stillCurrent` lets it stop cleanly
// if the user signs out or switches accounts mid-heal.
async function healMissingTakeaways(
  data: Record<string, unknown>,
  persistIfAbsent: (key: string, value: unknown) => void,
  stillCurrent: () => boolean
): Promise<void> {
  const targets = sessionsNeedingTakeaway({
    completed: data[KEYS.completed],
    hasTakeaway: (id) => KEYS.takeaway(id) in data,
    getConversation: (id) => {
      const c = data[KEYS.conversation(id)];
      return Array.isArray(c) ? (c as ConversationMessage[]) : null;
    },
  });

  for (const { id, moduleTitle, messages } of targets) {
    ...
    try {
      ...
      const text = (result.takeaway ?? "").trim();
      if (!text) continue;
      if (!stillCurrent()) return;
      const takeaway: Takeaway = {
        ...
        moduleTitle,
        text,
        textDirect: (result.takeawayDirect ?? "").trim() || undefined,
        savedAt: new Date().toISOString(),
      };
      persistIfAbsent(KEYS.takeaway(id), takeaway);
    } catch {
      // Best-effort — a transient failure just means we try again next load.
    }
  }
}
...
export function UserDataProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Non-blocking: quietly regenerate any takeaway that a wrap-up dropped, so
      // a completed session never stays takeaway-less. Writes each result only if
      // still absent, and stops if the user changes mid-heal.
      void healMissingTakeaways(
        data,
        (key, value) => {
          if (key in snapshotRef.current) return;
        ...
        () => !cancelled && loadedFor.current === userId
      );
    })();
