...
- "additions": new facts the person stated in conversation that aren't already on record. Each: {"category": one of [${FACT_CATEGORIES}], "label": the fact in their words (short), and optionally "domain" (for recurring_activity: Restore/Move/Think/Connect/Contribute), "description"}. Use one_off_dream for money-no-object/pipe dreams; aspiration for things they could realistically work toward; recurring_activity for regular activities — never mix these up. For recurring_activity, the "label" is the bare activity only (e.g. "sea swimming", "watching football", "a solo coffee and walk") — never fold in when, how often, or in which seasons it happens; put any such timing, frequency or seasonality in the "description". For keep_change_leave (the retired letter's stock-take of their current retirement), the "label" is the element of their life in their words and the "description" is exactly one of "keep", "change", or "leave" — whether they want to keep it as it is, reshape it, or let it go. If the person only floated something tentatively ("maybe", "possibly", "I might", "we'll see"), set "tentative": true on it, so it is recorded as a maybe and never treated as a settled plan or an anchor. Only include something clearly new and concrete. Empty array if nothing new.
